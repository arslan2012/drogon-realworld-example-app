                             ...
                             bool isForCreation)
{
    switch(index)
    {
        ...
        case 5:
            ...
            // asString().length() creates a string object, is there any better way to validate the length?
            if(pJson.isString() && pJson.asString().length() > 511)
            {
                err="String length exceeds limit for the " +
                    fieldName +
                    " field (the maximum value is 511)";
                return false;               
            }
